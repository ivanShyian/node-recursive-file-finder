...
const separator = '/'
const mainDir = '../inratingwebmobile/src/common-components'
const fileExtensions = ['vue', 'js', 'ts', 'vue.ts']
const listOfKeywords = [/api\./gm, /\$http\./gm]
const listOfFilesToExclude = ['api.ts']
const apiFilePath = '../inratingwebmobile/src/common-components/api/api.ts'
...
const runner = async (pathSource, files, regex, toExclude, separator, extraRegex, api) => {
  ...
  Object.keys(finalObj).sort().forEach(e => {
    sortedFinalObj[e] = finalObj[e]
  })
